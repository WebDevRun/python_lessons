...
# если находит экземпляры разделителя
...
print('Результат метода ljust(): ' + concat_str.ljust(20, '_'))

# Метод center(length, character) выровняет строку по центру, используя
# указанный символ (пробел по умолчанию) в качестве символа заполнения
print('Результат метода center(): ' + concat_str.center(20, '_'))

# Метод strip(characters) удаляет все символы
# (по умолчанию для удаления используется пробел).
txt = ',,,,,ssaaww.....banana,ssaaww.'

print('Результат метода strip(): ' + txt.strip(',.asw'))
...
# Метод endswith(value, start, end) возвращает значение true, если строка
# заканчивается указанным значением, в противном случае значение false, где:
# value - строка для поиска (обязательный аргумент)
# start - позиция начала поиска (опциональный аргумент; целое число;
# по умолчанию = 0)
# end - позиция конца поиска (опциональный аргумент, целое число;
...
print('Результат метода isdigit(): ' + str(null_str.isdigit()))
...
print('Результат метода isidentifier(): ' + str(txt.isidentifier()))
...
# в противном случае значение false
print('Результат метода islower(): ' + str(txt.islower()))

# Метод isupper() возвращает значение true, если все символы указаны
# в верхнем регистре, в противном случае значение false
upper_str = 'MY NAME IS PETER'

print('Результат метода isupper(): ' + str(upper_str.isupper()))

print('Результат метода islower(): ' + str(txt.islower()))

# Метод isnumeric() возвращает значение true, если все символы числовые (0-9),
# в противном случае значение false
print('Результат метода isnumeric() для строки ' +
      null_str + ': ' + str(null_str.isnumeric()))
